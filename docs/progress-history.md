...
2026-04-22 08:36 UTC | asked: debug Windows Jabber clients stuck on connecting | done: traced issue to plain c2s without working TLS, issued Let's Encrypt cert for `xmpp.memdecks.com`, switched production XMPP to STARTTLS on `5222`, mounted readable cert copies for the `ejabberd` container, and verified the public server now advertises STARTTLS with a valid chain | next: retry Gajim or another client, then test presence and direct messaging behavior
2026-04-22 08:58 UTC | asked: inspect Gajim warnings after STARTTLS fix | done: confirmed Gajim reaches and authenticates to `ejabberd`, traced the remaining failure to missing `mod_disco`, missing `mod_host_meta`, and the wrong TLS certificate on `xmpp.memdecks.com:443`, then added repo config for XMPP discovery modules and a dedicated nginx site for the XMPP hostname | next: deploy the XMPP discovery config live, re-test Gajim status, then verify direct message flow between demo users
2026-04-22 09:05 UTC | asked: investigate why demo users can message but still show offline | done: confirmed both demo accounts had empty rosters, identified this as normal XMPP subscription behavior, and added a helper script to create mutual roster entries so presence can flow in the thin-slice tests | next: link the demo users live, confirm presence updates in Gajim, then mark Jabber thin-slice connectivity complete
